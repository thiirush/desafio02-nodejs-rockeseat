...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const isPro = user.pro ? true : false;
  const userTodosCount = user.todos.length;

  if ((!isPro && userTodosCount < 10) || isPro) {
    return next();
  }

  return response.status(403).json({ error: "Limite máximo" });
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!isUUID) return response.status(400).json({ error: "Deu ruim" });
